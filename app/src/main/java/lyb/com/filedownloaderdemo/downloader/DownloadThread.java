	...
	public void run() {
		if(downloadedLength < block){//未下载完成
			try {
				...
				byte[] buffer = new byte[8192];	//设置本地数据缓存的大小为1M
				...
				while (!downloader.getExited() && (offset = inStream.read(buffer, 0, 8192)) != -1) {	//但用户没有要求停止下载，同时没有到达请求数据的末尾时候会一直循环读取数据
				...
				//Thread 1:java.net.UnknownHostException: Unable to resolve host "res.5253.com": No address associated with hostname
